...
export async function storeMessage(message: Omit<Message, 'id'>): Promise<number> {
  const db = getDatabase();
  return db.messages.add(message as Message);
}

export async function getMessagesWithUser(odD: string, limit = 100): Promise<Message[]> {
  const db = getDatabase();
  return db.messages
    .where('[odD+timestamp]')
...
export async function updateMessageStatus(id: number, status: Message['status']): Promise<void> {
  const db = getDatabase();
...
export async function markLastMessageAsFailed(odD: string): Promise<void> {
  ...
  const messages = await db.messages
    ...
    .between([odD, 0], [odD, Date.now()], true, true)
    .reverse()
    .filter((msg) => msg.direction === 'sent' && (msg.status === 'sent' || msg.status === 'pending'))
    .limit(1)
    .toArray();
